...
void app_main()
{
    ...
    // section number 0, inaddr (byte address) 1
    uint16_t sect_no = 0;
    uint16_t inaddr = 1;

    // write to first byte of address 0 here; hex value/bytes is 0x17

    uint8_t init_buf[1];
    uint8_t init_data = 0x17;
    memset(init_buf, 0, 1);
	memcpy(init_buf, &init_data, 1);
    int16_t init_result = W25Q64_pageWrite(&dev, sect_no, inaddr, init_buf, 1);
    ...
    printf("fast read\n");
    printf("fast read\n");
    int len =  W25Q64_fastread(&dev, addr, rbuf, READNUMBYTES);
    if (len != READNUMBYTES) {
